...
echo "=== 009-basic-credit-regulatory-slik (SDK Pipeline) ==="
...
# Trigger SLIK mapping — streams NDJSON, maps to OJK v2.1 format
RESP=$(curl -s --max-time 20 -X POST "$BASE/regulatory-stream" -H 'Content-Type: application/json' \
  -d '{"report_type":"slik-monthly"}')
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "nik\|kualitas_kredit\|slik\|nama" "response has SLIK mapped data"
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} SLIK pipeline returned data"
else
