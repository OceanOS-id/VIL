...
echo "=== 101c VilApp Multi Pipeline Benchmark ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Trigger pipeline
RESP=$(curl -s -X POST "$BASE/api/pipeline/trigger" -H 'Content-Type: application/json' \
  -d '{"prompt":"benchmark multi-pipeline"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "trigger has content"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("101c: POST /api/pipeline/trigger failed")
    echo -e "  ${RED}FAIL${NC} POST /api/pipeline/trigger empty"
fi
