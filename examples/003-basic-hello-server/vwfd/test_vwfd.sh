...
echo "=== 003-basic-hello-server (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/fx/convert
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/fx/convert" -H 'Content-Type: application/json' -d '{"from":"USD","to":"IDR","amount":100}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/fx/convert"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/fx/convert empty")
    echo -e "  ${RED}FAIL${NC} POST /api/fx/convert empty"
fi

# GET /api/fx/rates
RESP=$(curl -s --max-time 10 "$BASE/api/fx/rates")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/fx/rates"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/fx/rates empty")
    echo -e "  ${RED}FAIL${NC} GET /api/fx/rates empty"
fi

# GET /api/fx/stats
RESP=$(curl -s --max-time 10 "$BASE/api/fx/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/fx/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/fx/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/fx/stats empty"
fi
