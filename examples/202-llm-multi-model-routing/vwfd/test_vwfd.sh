...
echo "=== 202-llm-multi-model-routing (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/translate" -H 'Content-Type: application/json' -d '{"prompt":"Translate: hello"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/translate"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/translate empty"); echo -e "  ${RED}FAIL${NC} POST /api/translate empty"; }
