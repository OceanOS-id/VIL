...
echo "=== 202 LLM Multi-Model Routing ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Translate (requires simulator)
RESP=$(curl -s -X POST "$BASE/api/translate/translate" -H 'Content-Type: application/json' \
  -d '{"text":"Hello, how are you?","target_lang":"Indonesian"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "translated" "translate has translated"
    assert_output_contains "$RESP" "target_lang" "translate has target_lang"
    assert_output_contains "$RESP" "model_used" "translate has model_used"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("202: POST /api/translate/translate failed")
    echo -e "  ${RED}FAIL${NC} POST /api/translate/translate empty"
fi
