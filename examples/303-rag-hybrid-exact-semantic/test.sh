...
echo "=== 303 RAG Hybrid Exact+Semantic ==="
...
# Hybrid RAG — tries exact match first, falls back to semantic+LLM
RESP=$(curl -s -X POST "$BASE/api/hybrid" -H 'Content-Type: application/json' \
  -d '{"prompt":"How do I reset my password?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "hybrid has content"
    assert_output_contains "$RESP" "search_tier" "hybrid has search_tier"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("303: POST /api/hybrid empty")
    echo -e "  ${RED}FAIL${NC} POST /api/hybrid empty"
fi
