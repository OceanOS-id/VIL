...
echo "=== 008-basic-credit-quality-monitor (SDK Pipeline) ==="
...
# Trigger quality check — streams NDJSON, validates 5 rules per record
RESP=$(curl -s --max-time 20 -X POST "$BASE/quality-check" -H 'Content-Type: application/json' \
  -d '{"check":"full-scan"}')
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "kolektabilitas" "response has credit data"
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} quality monitor returned data"
else
