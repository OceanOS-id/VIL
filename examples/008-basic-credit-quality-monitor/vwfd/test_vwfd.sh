...
echo "=== 008-basic-credit-quality-monitor (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /quality-check
RESP=$(curl -s --max-time 10 -X POST "$BASE/quality-check" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /quality-check"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /quality-check empty")
    echo -e "  ${RED}FAIL${NC} POST /quality-check empty"
fi
