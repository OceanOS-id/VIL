...
echo "=== 304 RAG Citation Extraction ==="
...
# Cited RAG query
RESP=$(curl -s -X POST "$BASE/api/cited-rag" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is the termination clause?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "answer" "cited-rag has answer"
    assert_output_contains "$RESP" "citations" "cited-rag has citations"
    assert_output_contains "$RESP" "citation_count" "cited-rag has citation_count"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("304: POST /api/cited-rag empty")
    echo -e "  ${RED}FAIL${NC} POST /api/cited-rag empty"
fi
