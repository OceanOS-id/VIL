...
echo "=== 205-llm-chunked-summarizer (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/summarize" -H 'Content-Type: application/json' -d '{"prompt":"Summarize this text"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /summarize"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /summarize empty"); echo -e "  ${RED}FAIL${NC} POST /summarize empty"; }
