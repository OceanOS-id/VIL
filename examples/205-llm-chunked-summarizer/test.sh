...
echo "=== 205 LLM Chunked Summarizer ==="

# SDK pipeline — no /health, SSE streaming response
RESP=$(curl -sN --max-time 20 -X POST "$BASE/summarize" -H 'Content-Type: application/json' \
  -d '{"text":"Rust is a systems programming language focused on safety and performance. It prevents memory errors at compile time.","max_chunk_size":2000}')
if [ -n "$RESP" ] && [ ${#RESP} -gt 10 ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /summarize returned ${#RESP} chars"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("205: POST /summarize failed or empty")
    echo -e "  ${RED}FAIL${NC} POST /summarize empty"
fi
