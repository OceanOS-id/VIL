...
echo "=== 040-basic-auth-middleware-stack (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/admin/login
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/admin/login" -H 'Content-Type: application/json' -d '{"username":"admin","password":"secret"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/admin/login"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/admin/login empty")
    echo -e "  ${RED}FAIL${NC} POST /api/admin/login empty"
fi

# GET /api/protected/data
RESP=$(curl -s --max-time 10 "$BASE/api/protected/data")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/protected/data"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/protected/data empty")
    echo -e "  ${RED}FAIL${NC} GET /api/protected/data empty"
fi

# GET /api/public/info
RESP=$(curl -s --max-time 10 "$BASE/api/public/info")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/public/info"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/public/info empty")
    echo -e "  ${RED}FAIL${NC} GET /api/public/info empty"
fi
