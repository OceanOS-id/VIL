...
echo "=== 108-workflow-dag-scheduler ==="

PORT="${PORT:-8080}"
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/workflow/run
WORKFLOW_PAYLOAD='{"dag":"simple-pipeline"}'
assert_http_status_post "${BASE}/api/workflow/run" "$WORKFLOW_PAYLOAD" "200" \
    "POST /api/workflow/run returns HTTP 200"

RUN_RESP=$(curl -s --max-time 10 -X POST \
    -H "Content-Type: application/json" \
    -d "$WORKFLOW_PAYLOAD" \
    "${BASE}/api/workflow/run" 2>/dev/null)

assert_output_contains "$RUN_RESP" "status" \
    "Workflow run response contains status"

# Test: GET /api/workflow/status
assert_http_status "${BASE}/api/workflow/status" "200" \
    "GET /api/workflow/status returns HTTP 200"
