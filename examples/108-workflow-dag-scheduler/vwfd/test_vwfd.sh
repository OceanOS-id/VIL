...
echo "=== 108-workflow-dag-scheduler (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /run
RESP=$(curl -s --max-time 10 -X POST "$BASE/run" -H 'Content-Type: application/json' -d '{"workflow":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /run"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /run empty")
    echo -e "  ${RED}FAIL${NC} POST /run empty"
fi
