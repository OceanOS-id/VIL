...
echo "=== 201 LLM Basic Chat ==="

# ── Infra check: AI simulator ──
...
# Usage stats (before any chat) — prefix("/api") so path is /api/usage
RESP=$(curl -s "$BASE/api/usage")
assert_output_contains "$RESP" "provider" "usage has provider"
assert_output_contains "$RESP" "total_requests" "usage has total_requests"
assert_output_contains "$RESP" "total_tokens" "usage has total_tokens"

# Chat — prefix("/api") so path is /api/chat
RESP=$(curl -s -X POST "$BASE/api/chat" -H 'Content-Type: application/json' \
  -d '{"prompt":"I have chest pain and shortness of breath"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "chat has content"
    assert_output_contains "$RESP" "model" "chat has model"
    assert_output_contains "$RESP" "prompt_tokens_approx" "chat has prompt_tokens_approx"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("201: POST /api/chat failed")
    echo -e "  ${RED}FAIL${NC} POST /api/chat empty"
fi
