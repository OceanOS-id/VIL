...
echo "=== 034-basic-blocking-task (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST 
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/risk-engine/risk/assess" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
...
# GET /api/risk-engine/risk/health
RESP=$(curl -s --max-time 10 "$BASE/api/risk-engine/risk/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/risk-engine/risk/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/risk-engine/risk/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/risk-engine/risk/health empty"
fi
