...
echo "=== 034 Blocking Task ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Risk health
assert_http_status "$BASE/api/risk-engine/risk/health" 200 "risk health endpoint"

# Risk assessment (spawn_blocking Monte Carlo)
RESP=$(curl -s -X POST "$BASE/api/risk-engine/risk/assess" -H 'Content-Type: application/json' \
  -d '{"applicant_id":12345,"annual_income_cents":7500000,"debt_cents":2500000,"credit_history_months":60,"simulations":1000}')
assert_output_contains "$RESP" "score" "assess has score"
assert_output_contains "$RESP" "default_probability" "assess has default_probability"
assert_output_contains "$RESP" "simulations_run" "assess has simulations_run"
assert_output_contains "$RESP" "risk_tier" "assess has risk_tier"
assert_output_contains "$RESP" "exec_class" "assess has exec_class"
