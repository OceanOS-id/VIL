...
echo "=== 036 SSE Event Builder ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Info endpoint
assert_http_status "$BASE/api/ticker/info" 200 "info endpoint"
RESP=$(curl -s "$BASE/api/ticker/info")
assert_output_contains "$RESP" "Stock Market" "info has service name"
assert_output_contains "$RESP" "event_types" "info has event_types"

# SSE stream (read first 3 seconds then kill)
RESP=$(timeout 3 curl -s "$BASE/api/ticker/stream" 2>/dev/null || true)
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} SSE stream produces events"
else
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} SSE stream endpoint reachable"
fi
