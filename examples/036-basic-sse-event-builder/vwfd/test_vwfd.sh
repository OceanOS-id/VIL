...
echo "=== 036-basic-sse-event-builder (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/ticker/info
RESP=$(curl -s --max-time 10 "$BASE/api/ticker/info")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/ticker/info"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/ticker/info empty")
    echo -e "  ${RED}FAIL${NC} GET /api/ticker/info empty"
fi

# GET /api/ticker/stream
RESP=$(curl -s --max-time 10 "$BASE/api/ticker/stream")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/ticker/stream"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/ticker/stream empty")
    echo -e "  ${RED}FAIL${NC} GET /api/ticker/stream empty"
fi
