...
echo "=== 308-rag-full-pipeline-ingest-query (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/rag/query" -H 'Content-Type: application/json' -d '{"question":"What is VIL?"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/rag/query"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/rag/query empty"); echo -e "  ${RED}FAIL${NC} POST /api/rag/query empty"; }
