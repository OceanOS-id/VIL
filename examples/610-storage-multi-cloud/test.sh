...
echo "=== 610-storage-multi-cloud ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/storage/upload
UPLOAD_PAYLOAD='{"filename":"test.txt","content_base64":"aGVsbG8=","content_type":"text/plain","provider":"s3"}'
assert_http_status_post "${BASE}/api/storage/upload" "$UPLOAD_PAYLOAD" "200" \
    "POST /api/storage/upload returns HTTP 200"

# Test: GET /api/storage/list
assert_http_status "${BASE}/api/storage/list" "200" \
    "GET /api/storage/list returns HTTP 200"
