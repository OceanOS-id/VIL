...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/clickhouse/batch" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/clickhouse/batch"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/clickhouse/batch"); echo -e "  ${RED}FAIL${NC} POST /api/clickhouse/batch empty"; }
