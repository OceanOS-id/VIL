...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/auth/token" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/auth/token"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); echo -e "  ${RED}FAIL${NC} POST /api/auth/token empty"; }
