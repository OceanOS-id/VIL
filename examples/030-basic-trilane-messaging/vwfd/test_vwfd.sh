...
echo "=== 030-basic-trilane-messaging (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/gateway/orders/place
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/gateway/orders/place" -H 'Content-Type: application/json' -d '{"customer_id":"C1","product_id":"P1","quantity":2,"amount_cents":20000}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/gateway/orders/place"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/gateway/orders/place empty")
    echo -e "  ${RED}FAIL${NC} POST /api/gateway/orders/place empty"
fi
