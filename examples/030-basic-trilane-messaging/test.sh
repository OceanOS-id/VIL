...
echo "=== 030 Tri-Lane Messaging ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Place order (triggers Tri-Lane: trigger + data + control)
RESP=$(curl -s -X POST "$BASE/api/gateway/orders/place" -H 'Content-Type: application/json' \
  -d '{"customer_id":1,"product_id":42,"quantity":2,"amount_cents":4999}')
assert_output_contains "$RESP" "order_id" "order has order_id"
assert_output_contains "$RESP" "customer_id" "order has customer_id"
assert_output_contains "$RESP" "fulfillment_notified" "order has fulfillment_notified"
assert_output_contains "$RESP" "inventory_reserved" "order has inventory_reserved"
