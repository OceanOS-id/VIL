...
echo "=== 046-basic-mesh-scatter-gather (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/gateway/search/flights
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/gateway/search/flights" -H 'Content-Type: application/json' -d '{"origin":"CGK","destination":"SIN","date":"2026-05-01","passengers":1}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/gateway/search/flights"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/gateway/search/flights empty")
    echo -e "  ${RED}FAIL${NC} POST /api/gateway/search/flights empty"
fi
