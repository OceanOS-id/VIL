...
echo "=== 046-basic-mesh-scatter-gather ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/gateway/search/flights
FLIGHTS_PAYLOAD=$(cat "$SCRIPT_DIR/payloads/search-flights.json")
assert_http_status_post "${BASE}/api/gateway/search/flights" "$FLIGHTS_PAYLOAD" "200" \
    "POST /api/gateway/search/flights returns HTTP 200"

FLIGHTS_RESP=$(curl -s --max-time 10 -X POST \
    -H "Content-Type: application/json" \
    -d "$FLIGHTS_PAYLOAD" \
    "${BASE}/api/gateway/search/flights" 2>/dev/null)

assert_output_contains "$FLIGHTS_RESP" "results" \
    "Flights response contains results"
