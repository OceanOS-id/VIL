...
echo "=== 407-agent-multi-agent-orchestration ==="
echo "NOTE: This test requires ai-endpoint-simulator to be running."

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/support/ask
ASK_PAYLOAD='{"question":"How do I reset my password?"}'
assert_http_status_post "${BASE}/api/support/ask" "$ASK_PAYLOAD" "200" \
    "POST /api/support/ask returns HTTP 200"

ASK_RESP=$(curl -s --max-time 10 -X POST \
    -H "Content-Type: application/json" \
    -d "$ASK_PAYLOAD" \
    "${BASE}/api/support/ask" 2>/dev/null)

assert_output_contains "$ASK_RESP" "answer" \
    "Support ask response contains answer"

# Test: GET /api/support/tiers
assert_http_status "${BASE}/api/support/tiers" "200" \
    "GET /api/support/tiers returns HTTP 200"
