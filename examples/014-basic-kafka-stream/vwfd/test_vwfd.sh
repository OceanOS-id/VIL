...
echo "=== 014-basic-kafka-stream (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/kafka/bridge
RESP=$(curl -s --max-time 10 "$BASE/api/kafka/bridge")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/kafka/bridge"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/kafka/bridge empty")
    echo -e "  ${RED}FAIL${NC} GET /api/kafka/bridge empty"
fi

# GET /api/kafka/config
RESP=$(curl -s --max-time 10 "$BASE/api/kafka/config")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/kafka/config"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/kafka/config empty")
    echo -e "  ${RED}FAIL${NC} GET /api/kafka/config empty"
fi

# GET /api/kafka/consumer
RESP=$(curl -s --max-time 10 "$BASE/api/kafka/consumer")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/kafka/consumer"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/kafka/consumer empty")
    echo -e "  ${RED}FAIL${NC} GET /api/kafka/consumer empty"
fi

# POST /api/kafka/produce
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/kafka/produce" -H 'Content-Type: application/json' -d '{"topic":"test","key":"k1","value":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/kafka/produce"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/kafka/produce empty")
    echo -e "  ${RED}FAIL${NC} POST /api/kafka/produce empty"
fi
