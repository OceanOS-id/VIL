...
use vil_log::types::*;
use vil_log::{app_log, access_log, LogConfig, LogLevel};
...
async fn main() {
    ...
    println!("  ║          Total: {} events per test | --release                       ║", TOTAL_EVENTS);
    println!("  ╚══════════════════════════════════════════════════════════════════════════════╝");
    ...
    println!("  {}", "─".repeat(62));
    for r in &results {
        println!("  {:<10} {:>16.0} {:>16.0} {:>16.0}",
            r.threads, ns_per(r.tracing_dur), ns_per(r.vil_app_dur), ns_per(r.vil_flat_dur));
    }
    ...
    println!();
    println!("  {:<10} {:>18} {:>18}", "Threads", "VIL app_log!", "VIL access_log!");
    println!("  {}", "─".repeat(48));
    for r in &results {
    ...
    println!("  ├─────────────────────────────────────────────────────────────────────┤");
    println!("  │  VIL auto-detected {} CPU cores → {} striped SPSC rings.{} │",
        stripe_count, stripe_count,
        " ".repeat(14usize.saturating_sub(format!("{}{}", stripe_count, stripe_count).len())));
    println!("  │  Each thread selects ring via thread_id %% {}.                   {}│",
        stripe_count, " ".repeat(5usize.saturating_sub(format!("{}", stripe_count).len())));
    println!("  │                                                                     │");
    println!("  │  At ≤{} threads: ~1 thread/ring → zero contention.            {}│",
        stripe_count, " ".repeat(6usize.saturating_sub(format!("{}", stripe_count).len())));
    println!("  │  At >{} threads: sharing starts → some CAS contention.        {}│",
        stripe_count, " ".repeat(6usize.saturating_sub(format!("{}", stripe_count).len())));
    println!("  │                                                                     │");
