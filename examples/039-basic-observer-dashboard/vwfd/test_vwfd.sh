...
echo "=== 039-basic-observer-dashboard (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/demo/echo
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/demo/echo" -H 'Content-Type: application/json' -d '{"msg":"bench"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/demo/echo"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/demo/echo empty")
    echo -e "  ${RED}FAIL${NC} POST /api/demo/echo empty"
fi

# GET /api/demo/hello
RESP=$(curl -s --max-time 10 "$BASE/api/demo/hello")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/demo/hello"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/demo/hello empty")
    echo -e "  ${RED}FAIL${NC} GET /api/demo/hello empty"
fi
