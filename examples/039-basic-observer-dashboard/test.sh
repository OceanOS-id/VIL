...
echo "=== 039 Observer Dashboard ==="

assert_http_status "$BASE/health" 200 "health endpoint"

# Hello
RESP=$(curl -s "$API/hello")
assert_output_contains "$RESP" "Hello" "hello response"

# Echo
RESP=$(curl -s -X POST "$API/echo" -H 'Content-Type: application/json' \
  -d '{"test":"observer"}')
assert_output_contains "$RESP" "received_bytes" "echo has received_bytes"
assert_output_contains "$RESP" "echo" "echo has echo field"

# Observer dashboard (HTML)
STATUS=$(curl -s -o /dev/null -w "%{http_code}" "$BASE/_vil/dashboard/" --max-time 5)
if [ "$STATUS" = "200" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} observer dashboard loads"
else
    echo -e "  ${YELLOW}SKIP${NC} observer dashboard (may need OBSERVER=1)"
