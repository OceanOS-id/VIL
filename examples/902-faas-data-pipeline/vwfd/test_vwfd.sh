...
echo "=== 902-faas-data-pipeline (VWFD) ==="
# Health
...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/data/process" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/data/process"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); echo -e "  ${RED}FAIL${NC} POST /api/data/process empty"; }
