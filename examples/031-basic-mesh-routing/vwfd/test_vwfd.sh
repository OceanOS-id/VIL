...
echo "=== 031-basic-mesh-routing (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/teller/ping
RESP=$(curl -s --max-time 10 "$BASE/api/teller/ping")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/teller/ping"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/teller/ping empty")
    echo -e "  ${RED}FAIL${NC} GET /api/teller/ping empty"
fi

# POST /api/teller/submit
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/teller/submit" -H 'Content-Type: application/json' -d '{"data":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/teller/submit"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/teller/submit empty")
    echo -e "  ${RED}FAIL${NC} POST /api/teller/submit empty"
fi
