...
echo "=== 031 Mesh Routing ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Teller ping
RESP=$(curl -s "$BASE/api/teller/ping")
assert_output_contains "$RESP" "teller" "ping has teller service"
assert_output_contains "$RESP" "mesh_routes" "ping has mesh_routes"

# Submit transaction (teller → fraud → core_banking → notification)
RESP=$(curl -s -X POST "$BASE/api/teller/submit" -H 'Content-Type: application/json' \
  -d '{"from_account":"ACC-001","to_account":"ACC-002","amount_cents":50000,"currency":"USD"}')
assert_output_contains "$RESP" "fraud_score" "submit has fraud_score"
assert_output_contains "$RESP" "is_approved" "submit has is_approved"
assert_output_contains "$RESP" "transaction_ref" "submit has transaction_ref"
