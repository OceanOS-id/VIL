...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/publish" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/publish"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/publish"); echo -e "  ${RED}FAIL${NC} POST /api/publish empty"; }
