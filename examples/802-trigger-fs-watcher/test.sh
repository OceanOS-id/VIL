...
echo "=== 802 Trigger FS Watcher ==="

# If lifecycle already ran as CLI, check that log
if [ -n "${VIL_CLI_LOG:-}" ] && [ -f "$VIL_CLI_LOG" ]; then
    EXIT=${VIL_CLI_EXIT:-1}
    # 124 = timeout (expected for long-running watcher), 0 = clean, 143 = SIGTERM
    if [ "$EXIT" -eq 0 ] || [ "$EXIT" -eq 124 ] || [ "$EXIT" -eq 143 ]; then
        PASS_COUNT=$((PASS_COUNT + 1))
        echo -e "  ${GREEN}PASS${NC} watcher ran (exit=$EXIT)"
    else
        FAIL_COUNT=$((FAIL_COUNT + 1))
        FAILURES+=("802: unexpected exit=$EXIT")
        echo -e "  ${RED}FAIL${NC} unexpected exit=$EXIT"
    fi
    if [ -s "$VIL_CLI_LOG" ]; then
        assert_contains "$VIL_CLI_LOG" "watch\|watcher\|802\|fs\|filesystem" "output mentions watcher"
    fi
else
