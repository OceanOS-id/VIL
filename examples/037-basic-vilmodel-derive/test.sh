...
echo "=== 037 VilModel Derive ==="

assert_http_status "$BASE/health" 200 "health endpoint"

# Sample claim
RESP=$(curl -s "$API/claims/sample")
assert_output_contains "$RESP" "claim" "claims sample endpoint"
assert_output_contains "$RESP" "shm_bytes_len" "sample has shm_bytes_len"

# Submit claim
RESP=$(curl -s -X POST "$API/claims/submit" -H 'Content-Type: application/json' \
  -d '{"claim_id":1001,"policy_id":5001,"amount_cents":250000,"claim_type":"auto_collision","description":"Rear-end collision on highway"}')
assert_output_contains "$RESP" "claim_id" "submit has claim_id"
assert_output_contains "$RESP" "adjuster_assignment" "submit has adjuster_assignment"
assert_output_contains "$RESP" "estimated_processing_days" "submit has estimated_processing_days"
