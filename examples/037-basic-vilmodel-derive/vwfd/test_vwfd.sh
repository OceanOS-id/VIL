...
echo "=== 037-basic-vilmodel-derive (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/claims/claims/sample
RESP=$(curl -s --max-time 10 "$BASE/api/claims/claims/sample")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/claims/claims/sample"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/claims/claims/sample empty")
    echo -e "  ${RED}FAIL${NC} GET /api/claims/claims/sample empty"
fi

# POST 
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/claims/claims/submit" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
