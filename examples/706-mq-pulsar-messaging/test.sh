...
echo "=== 706-mq-pulsar-messaging ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/events/publish (ServiceProcess "events" + endpoint "/publish")
PUBLISH_PAYLOAD='{"event_type":"payment","transaction_id":"TXN-001","amount":250.00,"currency":"USD","from_account":"ACC-1234","to_account":"ACC-5678"}'
assert_http_status_post "${BASE}/api/events/publish" "$PUBLISH_PAYLOAD" "200" \
    "POST /api/events/publish returns HTTP 200"

# Test: GET /api/events/stats
assert_http_status "${BASE}/api/events/stats" "200" \
    "GET /api/events/stats returns HTTP 200"
