...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/events/publish" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/events/publish"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/events/publish"); echo -e "  ${RED}FAIL${NC} POST /api/events/publish empty"; }
