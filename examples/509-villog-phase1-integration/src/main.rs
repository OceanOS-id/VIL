...
async fn main() {
    ...
    println!();
    println!("  Drain received: {} total events, {} db_log events", total, db_count);
    if db_count >= 6 {
    ...
    println!();
    println!("  ── Part 2: db_log! Throughput ({} events) ──", BENCH_EVENTS);
    println!();
    ...
    println!("  db_log! latency:     {:.0}ns (1T), {:.0}ns (4T)", ns_1, ns_4);
    println!("  db_log! throughput:  {:.2} M/s (1T), {:.2} M/s (4T)", mps_1, mps_4);
    println!();
    ...
    if all_pass {
        ...
        if db_count < 6 { println!("  ║  • Missing db_log events ({}/6)                    ║", db_count); }
        if drops > 0 { println!("  ║  • Ring drops: {}                                   ║", drops); }
        if ns_1 >= 500.0 { println!("  ║  • Latency over budget: {:.0}ns                      ║", ns_1); }
        println!("  ╚══════════════════════════════════════════════════════╝");
    }
