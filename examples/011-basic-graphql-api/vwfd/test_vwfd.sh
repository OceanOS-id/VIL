...
echo "=== 011-basic-graphql-api (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/graphql/entities
RESP=$(curl -s --max-time 10 "$BASE/api/graphql/entities")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/graphql/entities"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/graphql/entities empty")
    echo -e "  ${RED}FAIL${NC} GET /api/graphql/entities empty"
fi

# POST /api/graphql/query
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/graphql/query" -H 'Content-Type: application/json' -d '{"query":"{ orders { id } }"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/graphql/query"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/graphql/query empty")
    echo -e "  ${RED}FAIL${NC} POST /api/graphql/query empty"
fi

# GET /api/graphql/schema
RESP=$(curl -s --max-time 10 "$BASE/api/graphql/schema")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/graphql/schema"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/graphql/schema empty")
    echo -e "  ${RED}FAIL${NC} GET /api/graphql/schema empty"
fi
