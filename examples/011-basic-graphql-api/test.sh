...
API="$BASE/api/graphql"

echo "=== 011 GraphQL API ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Schema
assert_http_status "$API/schema" 200 "schema endpoint"

# Entities
assert_http_status "$API/entities" 200 "entities endpoint"
RESP=$(curl -s "$API/entities")
assert_output_contains "$RESP" "Order" "entities has Order"
assert_output_contains "$RESP" "Customer" "entities has Customer"

# Query orders (standard binary uses in-memory pattern matching on query string)
QUERY='{"query":"{ orders { id total status } }"}'
RESP=$(curl -s -X POST "$API/query" -H 'Content-Type: application/json' -d "$QUERY")
assert_output_contains "$RESP" "orders" "query returns orders"
assert_output_contains "$RESP" "total" "query has total field"

# Query customers
QUERY='{"query":"{ customers { id name tier } }"}'
RESP=$(curl -s -X POST "$API/query" -H 'Content-Type: application/json' -d "$QUERY")
assert_output_contains "$RESP" "customers" "query returns customers"
assert_output_contains "$RESP" "name" "query has name field"
