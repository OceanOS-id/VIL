...
echo "=== 020-basic-ai-ab-testing (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/ab/config
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/ab/config" -H 'Content-Type: application/json' -d '{"split_percentage":60}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/ab/config"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/ab/config empty")
    echo -e "  ${RED}FAIL${NC} POST /api/ab/config empty"
fi

# POST /api/ab/infer
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/ab/infer" -H 'Content-Type: application/json' -d '{"prompt":"test","max_cost_usd":0.01}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/ab/infer"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/ab/infer empty")
    echo -e "  ${RED}FAIL${NC} POST /api/ab/infer empty"
fi

# GET /api/ab/metrics
RESP=$(curl -s --max-time 10 "$BASE/api/ab/metrics")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/ab/metrics"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/ab/metrics empty")
    echo -e "  ${RED}FAIL${NC} GET /api/ab/metrics empty"
fi
