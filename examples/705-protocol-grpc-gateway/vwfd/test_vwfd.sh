...
RESP=$(curl -s --max-time 10 -X POST "$BASE/charge" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /charge"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /charge"); echo -e "  ${RED}FAIL${NC} POST /charge empty"; }
