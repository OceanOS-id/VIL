...
echo "=== 032-basic-failover-ha (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST 
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/backup/charge" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST "
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST  empty")
    echo -e "  ${RED}FAIL${NC} POST  empty"
fi

# GET /api/backup/health
RESP=$(curl -s --max-time 10 "$BASE/api/backup/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/backup/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/backup/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/backup/health empty"
fi

# POST 
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/primary/charge" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST "
else
...
# GET /api/primary/health
RESP=$(curl -s --max-time 10 "$BASE/api/primary/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/primary/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/primary/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/primary/health empty"
fi
