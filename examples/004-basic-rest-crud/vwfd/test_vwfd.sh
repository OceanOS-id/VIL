...
echo "=== 004-basic-rest-crud (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/tasks/tasks
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/tasks/tasks" -H 'Content-Type: application/json' -d '{"title":"Test Task","description":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/tasks/tasks"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/tasks/tasks empty")
    echo -e "  ${RED}FAIL${NC} POST /api/tasks/tasks empty"
fi

# DELETE /api/tasks/tasks/
RESP=$(curl -s --max-time 10 -X DELETE "$BASE/api/tasks/tasks/" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} DELETE /api/tasks/tasks/"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("DELETE /api/tasks/tasks/ empty")
    echo -e "  ${RED}FAIL${NC} DELETE /api/tasks/tasks/ empty"
fi

# GET /api/tasks/tasks/
RESP=$(curl -s --max-time 10 "$BASE/api/tasks/tasks/")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/tasks/tasks/"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/tasks/tasks/ empty")
    echo -e "  ${RED}FAIL${NC} GET /api/tasks/tasks/ empty"
fi
...
RESP=$(curl -s --max-time 10 "$BASE/api/tasks/tasks/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/tasks/tasks/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/tasks/tasks/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/tasks/tasks/stats empty"
fi

# PUT /api/tasks/tasks/
RESP=$(curl -s --max-time 10 -X PUT "$BASE/api/tasks/tasks/" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} PUT /api/tasks/tasks/"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("PUT /api/tasks/tasks/ empty")
    echo -e "  ${RED}FAIL${NC} PUT /api/tasks/tasks/ empty"
fi
