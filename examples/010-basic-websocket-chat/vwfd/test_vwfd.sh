...
echo "=== 010-basic-websocket-chat (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/chat/stats
RESP=$(curl -s --max-time 10 "$BASE/api/chat/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/chat/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/chat/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/chat/stats empty"
fi

# POST /api/chat/send
STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -X POST "$BASE/api/chat/send" -H 'Content-Type: application/json' -d '{"message":"test"}')
if [ "$STATUS" = "200" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/chat/send → 200"
else
