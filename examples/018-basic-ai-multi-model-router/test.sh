...
echo "=== 018 AI Multi-Model Router ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# List models
RESP=$(curl -s "$BASE/api/router/models")
assert_output_contains "$RESP" "gpt-4" "models has gpt-4"
assert_output_contains "$RESP" "gpt-3.5-turbo" "models has gpt-3.5-turbo"
assert_output_contains "$RESP" "cost_per_1m_tokens" "models has cost info"

# Route inference (requires simulator)
RESP=$(curl -s -X POST "$BASE/api/router/route" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is Rust?","max_cost_usd":0.001}' --max-time 15)
STATUS=$?
if [ $STATUS -eq 0 ] && [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "route response has content"
    assert_output_contains "$RESP" "model_used" "route response has model_used"
    assert_output_contains "$RESP" "tier" "route response has tier"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("018: POST /api/router/route failed")
    echo -e "  ${RED}FAIL${NC} POST /api/router/route failed"
fi

# Router stats
RESP=$(curl -s "$BASE/api/router/stats")
assert_output_contains "$RESP" "total_requests" "stats has total_requests"
assert_output_contains "$RESP" "estimated_savings_pct" "stats has estimated_savings_pct"
