...
echo "=== 018-basic-ai-multi-model-router (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/router/models
RESP=$(curl -s --max-time 10 "$BASE/api/router/models")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/router/models"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/router/models empty")
    echo -e "  ${RED}FAIL${NC} GET /api/router/models empty"
fi

# POST /api/router/route
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/router/route" -H 'Content-Type: application/json' -d '{"prompt":"test","max_cost_usd":0.01}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/router/route"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/router/route empty")
    echo -e "  ${RED}FAIL${NC} POST /api/router/route empty"
fi

# GET /api/router/stats
RESP=$(curl -s --max-time 10 "$BASE/api/router/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/router/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/router/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/router/stats empty"
fi
