...
# Calculator agent
RESP=$(curl -s -X POST "$BASE/api/calc" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is 42 * 13?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "calc has content"
    assert_output_contains "$RESP" "tools_used" "calc has tools_used"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("401: POST /api/calc empty")
    echo -e "  ${RED}FAIL${NC} POST /api/calc empty"
fi
