...
echo "=== 401-agent-calculator (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/calc" -H 'Content-Type: application/json' -d '{"prompt":"What is 42 * 13?"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/calc"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/calc empty"); echo -e "  ${RED}FAIL${NC} POST /api/calc empty"; }
