...
echo "=== 005-basic-multiservice-mesh-ndjson (SDK Pipeline) ==="

PORT=${PORT:-3084}
BASE="http://localhost:${PORT}"

# Prerequisite: credit-data-simulator
if ! curl -sf --max-time 3 http://localhost:18081/health 2>/dev/null | grep -q "healthy"; then
    skip_test "credit-data-simulator not running on :18081"
    exit 0
fi

# Trigger pipeline — fetches NDJSON from credit-sim, enriches with risk category + LTV
RESP=$(curl -s --max-time 20 -X POST "$BASE/ingest" -H 'Content-Type: application/json' \
  -d '{"request":"stream-credits"}')
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "kolektabilitas" "response has credit data"
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} pipeline returned data"
else
    skip_test "pipeline response empty (SSE streaming)"
fi
