...
echo "=== 005-basic-multiservice-mesh-ndjson (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /ingest
RESP=$(curl -s --max-time 10 -X POST "$BASE/ingest" -H 'Content-Type: application/json' -d '{"prompt":"test","request":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /ingest"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /ingest empty")
    echo -e "  ${RED}FAIL${NC} POST /ingest empty"
fi
