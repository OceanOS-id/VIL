...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/villog-structured-events" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/villog-structured-events"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/villog-structured-events"); echo -e "  ${RED}FAIL${NC} POST /api/villog-structured-events empty"; }
