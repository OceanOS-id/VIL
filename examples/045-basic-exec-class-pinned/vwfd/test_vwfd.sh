...
echo "=== 045-basic-exec-class-pinned (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/sensor/sensor/process
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/sensor/sensor/process" -H 'Content-Type: application/json' -d '{"sensor_id":"S1","readings":[1.0,2.0,3.0]}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/sensor/sensor/process"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/sensor/sensor/process empty")
    echo -e "  ${RED}FAIL${NC} POST /api/sensor/sensor/process empty"
fi

# GET /api/sensor/sensor/stats
RESP=$(curl -s --max-time 10 "$BASE/api/sensor/sensor/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/sensor/sensor/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/sensor/sensor/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/sensor/sensor/stats empty"
fi
