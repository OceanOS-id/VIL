...
BASE="http://localhost:3091"

echo "=== 025 RAG Service ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# RAG query (requires simulator)
RESP=$(curl -s -X POST "$BASE/api/rag" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is Rust ownership?"}' --max-time 15)
STATUS=$?
if [ $STATUS -eq 0 ] && [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "rag response has content"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("025: POST /api/rag failed")
    echo -e "  ${RED}FAIL${NC} POST /api/rag failed"
fi
