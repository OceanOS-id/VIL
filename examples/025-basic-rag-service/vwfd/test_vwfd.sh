...
echo "=== 025-basic-rag-service (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/rag
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/rag" -H 'Content-Type: application/json' -d '{"prompt":"What is API?"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/rag"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/rag empty")
