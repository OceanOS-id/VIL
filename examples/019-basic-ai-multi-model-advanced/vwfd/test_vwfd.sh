...
echo "=== 019-basic-ai-multi-model-advanced (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/triage/assess
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/triage/assess" -H 'Content-Type: application/json' -d '{"symptoms":"headache","severity":"low"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/triage/assess"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/triage/assess empty")
    echo -e "  ${RED}FAIL${NC} POST /api/triage/assess empty"
fi

# GET /api/triage/stats
RESP=$(curl -s --max-time 10 "$BASE/api/triage/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/triage/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/triage/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/triage/stats empty"
fi
