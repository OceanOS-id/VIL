...
echo "=== 019 Healthcare Triage AI (Fallback Router) ==="

# POST /api/triage/assess
RESP=$(curl -s -X POST "$BASE/api/triage/assess" -H 'Content-Type: application/json' \
  -d '{"symptoms":"chest pain, shortness of breath","severity":"high"}' --max-time 15)
assert_output_contains "$RESP" "assessment" \
    "POST /api/triage/assess returns assessment"
assert_output_contains "$RESP" "provider_used" \
    "Response includes provider_used"
assert_output_contains "$RESP" "fallback_triggered" \
    "Response includes fallback_triggered"

# GET /api/triage/stats
assert_http_status "$BASE/api/triage/stats" "200" \
    "GET /api/triage/stats returns 200"
