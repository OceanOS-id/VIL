...
echo "=== 047-basic-custom-error-stack (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/banking/accounts
RESP=$(curl -s --max-time 10 "$BASE/api/banking/accounts")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/banking/accounts"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/banking/accounts empty")
    echo -e "  ${RED}FAIL${NC} GET /api/banking/accounts empty"
fi

# POST /api/banking/transfer
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/banking/transfer" -H 'Content-Type: application/json' -d '{"from_account":"ACC-1001","to_account":"ACC-1002","amount_cents":50000}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/banking/transfer"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/banking/transfer empty")
    echo -e "  ${RED}FAIL${NC} POST /api/banking/transfer empty"
fi
