...
echo "=== 406-agent-vil-handler-shm (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/detect" -H 'Content-Type: application/json' -d '{"card_token":"tok_4242","merchant":"Shop","amount_cents":50000,"country":"US","city":"NY","transactions_last_hour":2}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/detect"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/detect empty"); echo -e "  ${RED}FAIL${NC} POST /api/detect empty"; }
