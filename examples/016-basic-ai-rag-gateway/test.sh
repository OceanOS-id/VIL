...
echo "=== 016 AI RAG Gateway ==="

# SDK_PIPELINE SSE: check HTTP status (body is streaming)
STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 15 -X POST "$BASE/rag" \
  -H 'Content-Type: application/json' -d '{"prompt":"What is Rust ownership?"}')
assert_eq "$STATUS" "200" "POST /rag returns HTTP 200"
