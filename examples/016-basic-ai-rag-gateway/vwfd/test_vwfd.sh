...
echo "=== 016-basic-ai-rag-gateway (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /rag
RESP=$(curl -s --max-time 10 -X POST "$BASE/rag" -H 'Content-Type: application/json' -d '{"prompt":"What is API?"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /rag"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /rag empty")
    echo -e "  ${RED}FAIL${NC} POST /rag empty"
fi
