...
echo "=== 305-rag-guardrail-pipeline (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/safe-rag" -H 'Content-Type: application/json' -d '{"prompt":"test"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/safe-rag"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/safe-rag empty"); echo -e "  ${RED}FAIL${NC} POST /api/safe-rag empty"; }
