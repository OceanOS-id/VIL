...
echo "=== 305 RAG Guardrail Pipeline ==="
...
# Safe RAG query (NativeCode retrieval → Sidecar PII scan → NativeCode hallucination → LLM → respond)
RESP=$(curl -s -X POST "$BASE/api/safe-rag" -H 'Content-Type: application/json' \
  -d '{"prompt":"What are the symptoms of diabetes?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "safe-rag has content"
    assert_output_contains "$RESP" "confidence_score" "safe-rag has confidence_score"
    assert_output_contains "$RESP" "guardrail_status" "safe-rag has guardrail_status"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("305: POST /api/safe-rag empty")
    echo -e "  ${RED}FAIL${NC} POST /api/safe-rag empty"
fi
