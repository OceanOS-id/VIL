...
echo "=== 801 Trigger Cron Basic ==="

# CLI mode: lifecycle.sh runs binary and captures output+exit code
if [ -n "${VIL_CLI_LOG:-}" ] && [ -f "$VIL_CLI_LOG" ]; then
    EXIT=${VIL_CLI_EXIT:-1}
    assert_exit_code "$EXIT" 0 "cron binary exits cleanly after 3 fires"
    assert_contains "$VIL_CLI_LOG" "fire\|Cron\|trigger\|schedule\|801" "output contains cron activity"
else
