...
echo "=== 203-llm-code-review-with-tools (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/code/review" -H 'Content-Type: application/json' -d '{"code":"fn main() {}"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/code/review"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/code/review empty"); echo -e "  ${RED}FAIL${NC} POST /api/code/review empty"; }
