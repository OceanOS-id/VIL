...
RESP=$(curl -s -X POST "$BASE/api/code/review" -H 'Content-Type: application/json' \
  -d '{"code":"fn main() { println!(\"hello\"); }"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "review" "review has review field"
    assert_output_contains "$RESP" "tools_executed" "review has tools_executed"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("203: POST /api/code/review failed")
