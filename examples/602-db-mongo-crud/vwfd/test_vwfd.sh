...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/mongo/documents" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/mongo/documents"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/mongo/documents"); echo -e "  ${RED}FAIL${NC} POST /api/mongo/documents empty"; }
