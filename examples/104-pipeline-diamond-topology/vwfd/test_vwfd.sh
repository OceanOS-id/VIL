...
echo "=== 104-pipeline-diamond-topology (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /diamond
RESP=$(curl -s --max-time 10 -X POST "$BASE/diamond" -H 'Content-Type: application/json' -d '{"request":"summary"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /diamond"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /diamond empty")
    echo -e "  ${RED}FAIL${NC} POST /diamond empty"
fi
