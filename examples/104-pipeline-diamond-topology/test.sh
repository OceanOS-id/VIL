...
echo "=== 104 Pipeline Diamond Topology ==="

if nc -z localhost 3095 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} Summary port 3095"
    RESP=$(curl -s --max-time 15 -X POST http://localhost:3095/diamond -H 'Content-Type: application/json' \
      -d '{"request":"summary"}' 2>/dev/null)
    if [ -n "$RESP" ]; then
        assert_output_contains "$RESP" "_view" "diamond has _view marker"
    else
        FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("Summary stream empty")
        echo -e "  ${RED}FAIL${NC} Summary stream empty"
    fi
else
    skip_test "Summary :3095 not open"
fi

if nc -z localhost 3096 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} Detail port 3096"
else
    skip_test "Detail :3096 not open"
fi
