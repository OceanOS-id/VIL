...
echo "=== 022-basic-sidecar-python (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/credit/health
RESP=$(curl -s --max-time 10 "$BASE/api/credit/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/credit/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/credit/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/credit/health empty"
fi

# POST /api/credit/score
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/credit/score" -H 'Content-Type: application/json' -d '{"nik":"1234567890123456","income":10000000,"loan_amount":50000000,"employment_years":5,"existing_debt":5000000}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/credit/score"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/credit/score empty")
    echo -e "  ${RED}FAIL${NC} POST /api/credit/score empty"
fi
