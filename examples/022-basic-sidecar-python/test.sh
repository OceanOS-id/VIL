...
echo "=== 022-basic-sidecar-python (Credit Scoring) ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# POST /api/credit/score — credit scoring
RESP=$(curl -s -X POST "$BASE/api/credit/score" -H 'Content-Type: application/json' \
  -d '{"nik":"3201234567890001","income":15000000,"loan_amount":50000000,"employment_years":3,"existing_debt":5000000}' \
  --max-time 15)

assert_output_contains "$RESP" "score" \
    "POST /api/credit/score returns score"
assert_output_contains "$RESP" "risk_class" \
    "Response includes risk_class"
assert_output_contains "$RESP" "dti_ratio" \
    "Response includes dti_ratio"
assert_output_contains "$RESP" "recommendation" \
    "Response includes recommendation"
assert_output_contains "$RESP" "factors" \
    "Response includes factors"

# GET /api/credit/health
assert_http_status "$BASE/api/credit/health" "200" \
    "GET /api/credit/health returns 200"
