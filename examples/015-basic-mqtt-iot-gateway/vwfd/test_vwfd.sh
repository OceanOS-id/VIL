...
echo "=== 015-basic-mqtt-iot-gateway (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/sensors
RESP=$(curl -s --max-time 10 "$BASE/api/sensors")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/sensors"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/sensors empty")
    echo -e "  ${RED}FAIL${NC} GET /api/sensors empty"
fi

# GET /api/mqtt/config
RESP=$(curl -s --max-time 10 "$BASE/api/mqtt/config")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/mqtt/config"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/mqtt/config empty")
    echo -e "  ${RED}FAIL${NC} GET /api/mqtt/config empty"
fi

# GET /api/mqtt/topics
RESP=$(curl -s --max-time 10 "$BASE/api/mqtt/topics")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/mqtt/topics"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/mqtt/topics empty")
    echo -e "  ${RED}FAIL${NC} GET /api/mqtt/topics empty"
fi

# POST /api/sensors/data
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/sensors/data" -H 'Content-Type: application/json' -d '{"sensor":"temp-01","value":23.5}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/sensors/data"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/sensors/data empty")
    echo -e "  ${RED}FAIL${NC} POST /api/sensors/data empty"
fi
