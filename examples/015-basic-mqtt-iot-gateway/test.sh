...
echo "=== 015 MQTT IoT Gateway ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# MQTT config
assert_http_status "$BASE/api/mqtt/config" 200 "mqtt config endpoint"

# Sensor list
assert_http_status "$BASE/api/sensors" 200 "sensors list"

# MQTT topics
assert_http_status "$BASE/api/mqtt/topics" 200 "mqtt topics"

# Publish sensor data
SENSOR='{"sensor_id":"temp-001","type":"temperature","value":23.5,"unit":"celsius"}'
RESP=$(curl -s -X POST "$BASE/api/sensors/data" -H 'Content-Type: application/json' -d "$SENSOR")
assert_output_contains "$RESP" "published" "sensor data published"
assert_output_contains "$RESP" "mqtt_topic" "response has mqtt_topic"
