...
echo "=== 044-basic-graphql-subscriptions (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST 
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/notifications/notifications/publish" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
...
RESP=$(curl -s --max-time 10 "$BASE/api/notifications/notifications/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/notifications/notifications/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/notifications/notifications/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/notifications/notifications/stats empty"
fi
