...
echo "=== 307-rag-vectordb-knowledge-index ==="

PORT=${PORT:-3107}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/search/query
SEARCH_PAYLOAD='{"query":"password reset","top_k":3}'
assert_http_status_post "${BASE}/api/search/query" "$SEARCH_PAYLOAD" "200" \
    "POST /api/search/query returns HTTP 200"

SEARCH_RESP=$(curl -s --max-time 10 -X POST \
    -H "Content-Type: application/json" \
    -d "$SEARCH_PAYLOAD" \
    "${BASE}/api/search/query" 2>/dev/null)

assert_output_contains "$SEARCH_RESP" "results" \
    "Search response contains results"

# Test: GET /api/search/stats
assert_http_status "${BASE}/api/search/stats" "200" \
    "GET /api/search/stats returns HTTP 200"

STATS_RESP=$(curl -s --max-time 10 "${BASE}/api/search/stats" 2>/dev/null)

assert_output_contains "$STATS_RESP" "document_count" \
    "Stats response contains document_count"
