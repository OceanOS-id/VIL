...
echo "=== 307-rag-vectordb-knowledge-index (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/search/query" -H 'Content-Type: application/json' -d '{"question":"What is VIL?"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/search/query"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/search/query empty"); echo -e "  ${RED}FAIL${NC} POST /api/search/query empty"; }
