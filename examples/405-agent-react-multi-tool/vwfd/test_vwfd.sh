...
echo "=== 405-agent-react-multi-tool (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/react" -H 'Content-Type: application/json' -d '{"prompt":"Calculate 15% of 240"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/react"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/react empty"); echo -e "  ${RED}FAIL${NC} POST /api/react empty"; }
