...
echo "=== 405 Agent ReAct Multi-Tool ==="
...
# ReAct agent
RESP=$(curl -s -X POST "$BASE/api/react" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is 15% of 240 plus tax at 11%?"}' --max-time 30)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "answer" "react has answer"
    assert_output_contains "$RESP" "reasoning_trace" "react has reasoning_trace"
    assert_output_contains "$RESP" "iterations" "react has iterations"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("405: POST /api/react empty")
    echo -e "  ${RED}FAIL${NC} POST /api/react empty"
fi
