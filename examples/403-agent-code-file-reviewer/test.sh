...
echo "=== 403 Agent Code File Reviewer ==="
...
# Code review agent
RESP=$(curl -s -X POST "$BASE/api/code-review" -H 'Content-Type: application/json' \
  -d '{"prompt":"Review the main.rs file"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "code-review has content"
    assert_output_contains "$RESP" "tools_executed" "code-review has tools_executed"
    assert_output_contains "$RESP" "files_available" "code-review has files_available"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("403: POST /api/code-review empty")
    echo -e "  ${RED}FAIL${NC} POST /api/code-review empty"
fi
