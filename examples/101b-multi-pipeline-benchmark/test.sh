...
echo "=== 101b Multi Pipeline Benchmark ==="

# SDK_PIPELINE: no VilApp /health — port check only
...
# Trigger pipeline (SSE streaming response — need -N for unbuffered)
STATUS=$(curl -s -o /dev/null -w "%{http_code}" --max-time 10 -X POST "$BASE/trigger" \
  -H 'Content-Type: application/json' -d '{"request":"bench"}')
if [ "$STATUS" = "200" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /trigger returned HTTP 200 (SSE stream)"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("101b: trigger HTTP $STATUS")
    echo -e "  ${RED}FAIL${NC} POST /trigger HTTP $STATUS"
fi
