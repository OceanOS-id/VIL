...
echo "=== 021-basic-wasm-faas (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /wasm/pricing
RESP=$(curl -s --max-time 10 -X POST "$BASE/wasm/pricing" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /wasm/pricing"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /wasm/pricing empty")
    echo -e "  ${RED}FAIL${NC} POST /wasm/pricing empty"
fi
