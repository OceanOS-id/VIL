...
# Module inventory
RESP=$(curl -s --max-time 5 "$BASE/api/wasm-faas/wasm/modules")
assert_output_contains "$RESP" "pricing" "modules lists pricing"
assert_output_contains "$RESP" "validation" "modules lists validation"
assert_output_contains "$RESP" "transform" "modules lists transform"

# Pricing (call_i32)
RESP=$(curl -s -X POST "$BASE/api/wasm-faas/wasm/pricing" -H 'Content-Type: application/json' \
  -d '{"function":"calculate_price","args":[1000,15]}')
assert_output_contains "$RESP" "result" "pricing has result"
assert_output_contains "$RESP" "pricing" "pricing module identified"

# Validation (call_i32)
RESP=$(curl -s -X POST "$BASE/api/wasm-faas/wasm/validation" -H 'Content-Type: application/json' \
  -d '{"function":"validate_order","args":[500,1000]}')
assert_output_contains "$RESP" "result" "validation has result"

# Transform (call_with_memory via WASI stdin/stdout)
RESP=$(curl -s -X POST "$BASE/api/wasm-faas/wasm/transform" -H 'Content-Type: application/json' \
  -d '{"function":"to_uppercase","input":"hello vil"}')
assert_output_contains "$RESP" "result" "transform has result"
assert_output_contains "$RESP" "HELLO VIL\|transform" "transform output correct"
