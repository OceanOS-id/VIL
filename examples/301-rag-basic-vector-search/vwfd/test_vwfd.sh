...
echo "=== 301-rag-basic-vector-search (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/rag" -H 'Content-Type: application/json' -d '{"prompt":"What is API?"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/rag"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/rag empty"); echo -e "  ${RED}FAIL${NC} POST /api/rag empty"; }
