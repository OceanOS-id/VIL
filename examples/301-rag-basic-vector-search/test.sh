...
echo "=== 301 RAG Basic Vector Search ==="
...
# RAG query — prompt field, response has content + docs_used
RESP=$(curl -s -X POST "$BASE/api/rag" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is API authentication?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "rag has content"
    assert_output_contains "$RESP" "docs_used" "rag has docs_used"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("301: POST /api/rag empty")
    echo -e "  ${RED}FAIL${NC} POST /api/rag empty"
fi
