...
echo "=== 102-pipeline-fanout-scatter (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /npl
RESP=$(curl -s --max-time 10 -X POST "$BASE/npl" -H 'Content-Type: application/json' -d '{"request":"npl"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /npl"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /npl empty")
    echo -e "  ${RED}FAIL${NC} POST /npl empty"
fi
