...
RESP=$(curl -s --max-time 10 -X PUT "$BASE/api/storage/upload" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} PUT /api/storage/upload"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("PUT /api/storage/upload"); echo -e "  ${RED}FAIL${NC} PUT /api/storage/upload empty"; }
