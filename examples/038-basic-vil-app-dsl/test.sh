...
API="$BASE/api/restaurant-order-system"

echo "=== 038 VIL App DSL ==="

assert_http_status "$BASE/health" 200 "health endpoint"

# Menu
RESP=$(curl -s "$API/menu")
assert_output_contains "$RESP" "restaurant" "menu has restaurant"
assert_output_contains "$RESP" "items" "menu has items"

# Place order
RESP=$(curl -s -X POST "$API/order" -H 'Content-Type: application/json' \
  -d '{"table_number":5,"items":["Bruschetta","Margherita Pizza"]}')
assert_output_contains "$RESP" "order_id" "order has order_id"
assert_output_contains "$RESP" "total_cents" "order has total_cents"
assert_output_contains "$RESP" "status" "order has status"

# Order status
RESP=$(curl -s "$API/order/1")
assert_output_contains "$RESP" "order_id" "order status has order_id"
assert_output_contains "$RESP" "status" "order status has status"

# Kitchen status
RESP=$(curl -s "$API/kitchen/status")
assert_output_contains "$RESP" "chefs_on_duty" "kitchen has chefs_on_duty"
assert_output_contains "$RESP" "load_percent" "kitchen has load_percent"
