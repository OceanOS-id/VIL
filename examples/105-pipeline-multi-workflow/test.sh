...
echo "=== 105 Pipeline Multi-Workflow ==="

if nc -z localhost 3098 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} Credit workflow port 3098"
    RESP=$(curl -s --max-time 15 -X POST http://localhost:3098/credit -H 'Content-Type: application/json' \
      -d '{"request":"credits"}' 2>/dev/null)
    if [ -n "$RESP" ]; then
        assert_output_contains "$RESP" "kolektabilitas" "credit has data"
    else
        FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("Credit stream empty")
        echo -e "  ${RED}FAIL${NC} Credit stream empty"
    fi
else
    skip_test "Credit :3098 not open"
fi

if nc -z localhost 3097 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} AI workflow port 3097"
else
    skip_test "AI :3097 not open"
fi
