...
echo "=== 105-pipeline-multi-workflow (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /ai
RESP=$(curl -s --max-time 10 -X POST "$BASE/ai" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /ai"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /ai empty")
    echo -e "  ${RED}FAIL${NC} POST /ai empty"
fi
