...
echo "=== 806 Trigger IoT MQTT ==="

if [ -n "${VIL_CLI_LOG:-}" ] && [ -f "$VIL_CLI_LOG" ]; then
    EXIT=${VIL_CLI_EXIT:-1}
    # 124=timeout (expected, watcher waits 60s), 0=clean, 143=killed
    if [ "$EXIT" -eq 0 ] || [ "$EXIT" -eq 124 ] || [ "$EXIT" -eq 143 ]; then
        PASS_COUNT=$((PASS_COUNT + 1))
        echo -e "  ${GREEN}PASS${NC} IoT trigger ran (exit=$EXIT)"
    else
        FAIL_COUNT=$((FAIL_COUNT + 1))
        FAILURES+=("806: exit=$EXIT")
        echo -e "  ${RED}FAIL${NC} unexpected exit=$EXIT"
    fi
    # Check output shows MQTT connection/topic
    if [ -s "$VIL_CLI_LOG" ]; then
        assert_contains "$VIL_CLI_LOG" "MQTT\|mqtt\|factory\|sensor\|806\|Topic\|trigger" "output shows MQTT activity"
    fi
    # If mosquitto_pub available, publish test event
    if command -v mosquitto_pub >/dev/null 2>&1; then
        echo -e "  ${DIM}(mosquitto_pub available — full event test would run here)${NC}"
    fi
else
    skip_test "No CLI log — lifecycle did not run in cli mode"
fi
