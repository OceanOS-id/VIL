...
echo "=== 803 Trigger Webhook Receiver ==="

if [ -n "${VIL_CLI_LOG:-}" ] && [ -f "$VIL_CLI_LOG" ]; then
    EXIT=${VIL_CLI_EXIT:-1}
    assert_exit_code "$EXIT" 0 "webhook receiver exits cleanly after self-test"
    assert_contains "$VIL_CLI_LOG" "WEBHOOK\|status=200\|webhook" "output shows webhook events"
else
