...
echo "=== 106-pipeline-sse-standard-dialect (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /stream
RESP=$(curl -s --max-time 10 -X POST "$BASE/stream" -H 'Content-Type: application/json' -d '{"factory_id":"F-101","sensor_type":"temperature"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /stream"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /stream empty")
    echo -e "  ${RED}FAIL${NC} POST /stream empty"
fi
