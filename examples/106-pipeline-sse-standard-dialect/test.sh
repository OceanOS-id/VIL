...
echo "=== 106 Pipeline SSE Standard Dialect ==="

# SDK_PIPELINE: no VilApp /health — port check
...
# Stream (SSE response)
RESP=$(curl -s -X POST "$BASE/stream" -H 'Content-Type: application/json' \
  -d '{"factory_id":"F-101","sensor_type":"temperature"}' --max-time 10)
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /stream returned SSE data"
else
    echo -e "  ${YELLOW}SKIP${NC} POST /stream — needs upstream SSE source"
