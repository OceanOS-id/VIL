...
echo "=== 007-basic-credit-npl-filter (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /filter-npl
RESP=$(curl -s --max-time 10 -X POST "$BASE/filter-npl" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /filter-npl"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /filter-npl empty")
    echo -e "  ${RED}FAIL${NC} POST /filter-npl empty"
fi
