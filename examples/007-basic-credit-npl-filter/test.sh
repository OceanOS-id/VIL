...
echo "=== 007-basic-credit-npl-filter (SDK Pipeline) ==="
...
# Trigger NPL filter — streams NDJSON, filters kolektabilitas >= 3
RESP=$(curl -s --max-time 20 -X POST "$BASE/filter-npl" -H 'Content-Type: application/json' \
  -d '{"filter":"npl"}')
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "kolektabilitas" "response has credit data"
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} NPL filter returned data"
else
