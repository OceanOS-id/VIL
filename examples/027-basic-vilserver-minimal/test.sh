...
echo "=== 027 VilServer Minimal ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Ready
assert_http_status "$BASE/ready" 200 "ready endpoint"
...
# Echo (JSON body)
RESP=$(curl -s -X POST "$BASE/echo" -H 'Content-Type: application/json' \
  -d '{"msg":"hi"}')
assert_output_contains "$RESP" "received" "echo has received"
assert_output_contains "$RESP" "echo" "echo has echo field"

# Metrics
assert_http_status "$BASE/metrics" 200 "metrics endpoint"

# Info
assert_http_status "$BASE/info" 200 "info endpoint"
