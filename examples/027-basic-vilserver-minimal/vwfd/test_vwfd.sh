...
echo "=== 027-basic-vilserver-minimal (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /echo
RESP=$(curl -s --max-time 10 -X POST "$BASE/echo" -H 'Content-Type: application/json' -d '{"msg":"bench"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /echo"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /echo empty")
    echo -e "  ${RED}FAIL${NC} POST /echo empty"
fi

# GET /hello
RESP=$(curl -s --max-time 10 "$BASE/hello")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /hello"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /hello empty")
    echo -e "  ${RED}FAIL${NC} GET /hello empty"
fi
