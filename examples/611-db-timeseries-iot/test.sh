...
echo "=== 611-db-timeseries-iot ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Test: POST /api/sensors/write (ServiceProcess "sensors" + endpoint "/write")
SENSOR_PAYLOAD='{"sensor_id":"TEMP-001","value":22.5,"unit":"celsius","timestamp":1712300000}'
assert_http_status_post "${BASE}/api/sensors/write" "$SENSOR_PAYLOAD" "200" \
    "POST /api/sensors/write returns HTTP 200"

# Test: GET /api/sensors/query with time range
assert_http_status "${BASE}/api/sensors/query?sensor_id=HVAC-01&start=0&end=9999999999" "200" \
    "GET /api/sensors/query returns HTTP 200"
