...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/sensors/write" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/sensors/write"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/sensors/write"); echo -e "  ${RED}FAIL${NC} POST /api/sensors/write empty"; }
