...
echo "=== 028 SSE Hub Streaming ==="

assert_http_status "$BASE/health" 200 "health endpoint"

# Stats
assert_http_status "$API/stats" 200 "stats endpoint"
RESP=$(curl -s "$API/stats")
assert_output_contains "$RESP" "connected_clients" "stats has connected_clients"
assert_output_contains "$RESP" "total_events" "stats has total_events"

# Publish event
RESP=$(curl -s -X POST "$API/publish" -H 'Content-Type: application/json' \
  -d '{"message":"test bid"}')
assert_output_contains "$RESP" "published" "publish response has published"
