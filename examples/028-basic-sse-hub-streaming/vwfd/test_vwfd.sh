...
echo "=== 028-basic-sse-hub-streaming (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/events/publish
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/events/publish" -H 'Content-Type: application/json' -d '{"message":"test event"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/events/publish"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/events/publish empty")
    echo -e "  ${RED}FAIL${NC} POST /api/events/publish empty"
fi

# GET /api/events/stats
RESP=$(curl -s --max-time 10 "$BASE/api/events/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/events/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/events/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/events/stats empty"
fi
