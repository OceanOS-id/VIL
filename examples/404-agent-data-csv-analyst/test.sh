...
# CSV analyze
RESP=$(curl -s -X POST "$BASE/api/csv-analyze" -H 'Content-Type: application/json' \
  -d '{"question":"What is the average sales amount?","csv_data":"product,amount\nA,100\nB,200\nC,150"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "analysis" "csv-analyze has analysis"
    assert_output_contains "$RESP" "stats" "csv-analyze has stats"
    assert_output_contains "$RESP" "rows_processed" "csv-analyze has rows_processed"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("404: POST /api/csv-analyze empty")
    echo -e "  ${RED}FAIL${NC} POST /api/csv-analyze empty"
fi
