...
echo "=== 404-agent-data-csv-analyst (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/csv-analyze" -H 'Content-Type: application/json' -d '{"question":"Average?","csv_data":"a,b\n1,2"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/csv-analyze"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/csv-analyze empty"); echo -e "  ${RED}FAIL${NC} POST /api/csv-analyze empty"; }
