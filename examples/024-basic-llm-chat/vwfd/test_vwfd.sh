...
echo "=== 024-basic-llm-chat (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/chat
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/chat" -H 'Content-Type: application/json' -d '{"prompt":"hello"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/chat"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/chat empty")
