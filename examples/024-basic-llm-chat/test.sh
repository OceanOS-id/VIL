...
# Chat query (requires simulator)
RESP=$(curl -s -X POST "$BASE/api/chat" -H 'Content-Type: application/json' \
  -d '{"prompt":"What is Rust?"}' --max-time 15)
STATUS=$?
if [ $STATUS -eq 0 ] && [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "chat response has content"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("024: POST /api/chat failed")
    echo -e "  ${RED}FAIL${NC} POST /api/chat failed"
fi
