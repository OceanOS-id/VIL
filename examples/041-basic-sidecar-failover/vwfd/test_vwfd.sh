...
echo "=== 041-basic-sidecar-failover (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/score/health
RESP=$(curl -s --max-time 10 "$BASE/api/score/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/score/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/score/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/score/health empty"
fi

# POST /api/score/predict
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/score/predict" -H 'Content-Type: application/json' -d '{"features":[0.5,0.3,0.8],"model":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/score/predict"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/score/predict empty")
    echo -e "  ${RED}FAIL${NC} POST /api/score/predict empty"
fi
