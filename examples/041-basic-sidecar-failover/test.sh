...
echo "=== 041-basic-sidecar-failover ==="

PORT=${PORT:-8080}
BASE="http://127.0.0.1:${PORT}"

# Health — may fail if sidecar bridge panics at startup
HEALTH_STATUS=$(curl -s --max-time 5 -o /dev/null -w "%{http_code}" "${BASE}/api/score/health")
if [ "$HEALTH_STATUS" = "200" ]; then
    assert_eq "$HEALTH_STATUS" "200" "GET /api/score/health returns 200"
    HEALTH_RESP=$(curl -s --max-time 10 "${BASE}/api/score/health")
    assert_output_contains "$HEALTH_RESP" "circuit_state" "Health has circuit_state"
else
    skip_test "health not reachable — sidecar bridge may have failed at startup"
fi

# Predict — requires sidecar process
PREDICT_STATUS=$(curl -s --max-time 5 -o /dev/null -w "%{http_code}" -X POST \
    -H "Content-Type: application/json" \
    -d '{"features":[1,2,3]}' \
    "${BASE}/api/score/predict")
if [ "$PREDICT_STATUS" = "200" ]; then
    assert_eq "$PREDICT_STATUS" "200" "POST /api/score/predict returns 200"
else
    skip_test "predict — sidecar not available"
fi
