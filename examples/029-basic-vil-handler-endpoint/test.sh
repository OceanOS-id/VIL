...
echo "=== 029 VIL Handler Endpoint ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Plain handler
RESP=$(curl -s "$BASE/api/demo/plain")
assert_output_contains "$RESP" "message" "plain handler has message"
assert_output_contains "$RESP" "plain" "plain handler style"

# Handled (vil_handler macro)
RESP=$(curl -s "$BASE/api/demo/handled")
assert_output_contains "$RESP" "message" "handled has message"
assert_output_contains "$RESP" "request_id" "handled has request_id"

# Endpoint (vil_endpoint macro, computes input^2)
RESP=$(curl -s -X POST "$BASE/api/demo/endpoint" -H 'Content-Type: application/json' \
  -d '{"value":7}')
assert_output_contains "$RESP" "result" "endpoint has result"
assert_output_contains "$RESP" "49" "7^2 = 49"
