...
echo "=== 204 LLM Streaming Translator ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s -X POST "$BASE/api/translate/batch" -H 'Content-Type: application/json' \
  -d '{"texts":["Hello world","Good morning"],"target_lang":"Indonesian"}' --max-time 20)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "translations" "response has translations"
    assert_output_contains "$RESP" "total" "response has total"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("204: POST /api/translate/batch failed")
    echo -e "  ${RED}FAIL${NC} POST /api/translate/batch empty"
fi
