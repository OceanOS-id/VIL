...
echo "=== 402 Market Research Agent ==="

# GET /api/catalog/products — real product data (no LLM needed)
assert_http_status "$BASE/api/catalog/products" "200" \
    "GET /api/catalog/products returns 200"

PRODUCTS=$(curl -s "$BASE/api/catalog/products" --max-time 10)
assert_output_contains "$PRODUCTS" "Logitech" \
    "Products contains Logitech"
assert_output_contains "$PRODUCTS" "price_cents" \
    "Products contains price_cents"

# POST /api/research/analyze — agent endpoint exists and responds
STATUS=$(curl -s -o /dev/null -w "%{http_code}" -X POST "$BASE/api/research/analyze" \
  -H 'Content-Type: application/json' \
  -d '{"query":"Compare wireless mice prices"}' --max-time 30)

if [ "$STATUS" = "200" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/research/analyze returns 200"
elif [ "$STATUS" = "500" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/research/analyze reachable (500 = LLM format issue)"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("402: unexpected status $STATUS")
