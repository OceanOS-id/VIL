...
echo "=== 026-basic-ai-agent (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/helpdesk/ask
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/helpdesk/ask" -H 'Content-Type: application/json' -d '{"prompt":"hello"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/helpdesk/ask"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/helpdesk/ask empty")
    echo -e "  ${RED}FAIL${NC} POST /api/helpdesk/ask empty"
fi
