...
echo "=== 023-basic-hybrid-wasm-sidecar (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/orders/health
RESP=$(curl -s --max-time 10 "$BASE/api/orders/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/orders/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/orders/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/orders/health empty"
fi

# POST /api/orders/order
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/orders/order" -H 'Content-Type: application/json' -d '{"item":"Widget","qty":3,"base_cents":10000,"customer_id":"C-001"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/orders/order"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/orders/order empty")
    echo -e "  ${RED}FAIL${NC} POST /api/orders/order empty"
fi
