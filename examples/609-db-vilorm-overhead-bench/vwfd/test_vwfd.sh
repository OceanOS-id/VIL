...
RESP=$(curl -s --max-time 10 "$BASE/api/orm/items")
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} GET /api/orm/items"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("GET /api/orm/items"); echo -e "  ${RED}FAIL${NC} GET /api/orm/items empty"; }
