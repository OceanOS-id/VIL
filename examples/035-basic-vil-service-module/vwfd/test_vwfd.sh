...
echo "=== 035-basic-vil-service-module (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /patients/register
RESP=$(curl -s --max-time 10 -X POST "$BASE/patients/register" -H 'Content-Type: application/json' -d '{"name":"John Doe","date_of_birth":"1990-01-01","insurance_id":"INS-001"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /patients/register"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /patients/register empty")
    echo -e "  ${RED}FAIL${NC} POST /patients/register empty"
fi

# POST /appointments/schedule
RESP=$(curl -s --max-time 10 -X POST "$BASE/appointments/schedule" -H 'Content-Type: application/json' -d '{"patient_id":1063,"doctor_id":5,"department":"cardiology","date":"2026-05-01","time_slot":"10:00"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /appointments/schedule"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /appointments/schedule empty")
    echo -e "  ${RED}FAIL${NC} POST /appointments/schedule empty"
fi
