...
API="$BASE/api/hospital-appointment-system"

echo "=== 035 VIL Service Module ==="

assert_http_status "$BASE/health" 200 "health endpoint"

# Register patient
RESP=$(curl -s -X POST "$API/patients/register" -H 'Content-Type: application/json' \
  -d '{"name":"John Doe","date_of_birth":"1990-05-15","insurance_id":"INS-12345"}')
assert_output_contains "$RESP" "patient_id" "register has patient_id"
assert_output_contains "$RESP" "registration_status" "register has status"

# Schedule appointment
RESP=$(curl -s -X POST "$API/appointments/schedule" -H 'Content-Type: application/json' \
  -d '{"patient_id":1001,"doctor_id":301,"department":"cardiology","date":"2026-04-01","time_slot":"09:30"}')
assert_output_contains "$RESP" "appointment_id" "schedule has appointment_id"
assert_output_contains "$RESP" "department" "schedule has department"
