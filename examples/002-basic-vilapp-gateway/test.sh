...
echo "=== 002-basic-vilapp-gateway (VilApp) ==="
...
# Test: POST /api/trigger returns 200
assert_http_status_post "${BASE}/api/trigger" "$(cat "$PAYLOAD")" "200" \
    "POST /api/trigger returns HTTP 200"

# Test: response body contains "content" field
RESPONSE=$(curl -s --max-time 10 -X POST \
    -H "Content-Type: application/json" \
    -d @"$PAYLOAD" \
    "${BASE}/api/trigger" 2>/dev/null)

assert_output_contains "$RESPONSE" "content" \
    "POST /api/trigger response has content field"
