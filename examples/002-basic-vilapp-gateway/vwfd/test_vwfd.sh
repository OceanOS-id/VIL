...
echo "=== 002-basic-vilapp-gateway (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/trigger
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/trigger" -H 'Content-Type: application/json' -d '{"prompt":"test","request":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/trigger"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/trigger empty")
    echo -e "  ${RED}FAIL${NC} POST /api/trigger empty"
fi
