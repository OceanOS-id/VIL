...
echo "=== 206-llm-decision-routing (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/underwrite/assess" -H 'Content-Type: application/json' -d '{"symptoms":"headache","severity":"low"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/underwrite/assess"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/underwrite/assess empty"); echo -e "  ${RED}FAIL${NC} POST /api/underwrite/assess empty"; }
