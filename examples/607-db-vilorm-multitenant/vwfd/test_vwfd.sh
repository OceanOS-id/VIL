...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/tenants/:tenant_id/users" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/tenants/:tenant_id/users"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/tenants/:tenant_id/users"); echo -e "  ${RED}FAIL${NC} POST /api/tenants/:tenant_id/users empty"; }
