...
echo "=== 302-rag-multi-source-fanin (VWFD) ==="

assert_http_status "$BASE/health" 200 "health endpoint"

RESP=$(curl -s --max-time 15 -X POST "$BASE/api/multi-rag" -H 'Content-Type: application/json' -d '{"prompt":"test"}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/multi-rag"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /api/multi-rag empty"); echo -e "  ${RED}FAIL${NC} POST /api/multi-rag empty"; }
