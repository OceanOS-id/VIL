...
echo "=== 302 RAG Multi-Source Fan-In ==="
...
RESP=$(curl -s -X POST "$BASE/api/multi-rag" -H 'Content-Type: application/json' \
  -d '{"prompt":"How does VIL routing work?"}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "content" "multi-rag has content"
    assert_output_contains "$RESP" "sources_searched" "multi-rag has sources_searched"
    assert_output_contains "$RESP" "results_merged" "multi-rag has results_merged"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("302: POST /api/multi-rag empty")
    echo -e "  ${RED}FAIL${NC} POST /api/multi-rag empty"
fi
