...
echo "=== 103-pipeline-fanin-gather (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /gather
RESP=$(curl -s --max-time 10 -X POST "$BASE/gather" -H 'Content-Type: application/json' -d '{"request":"credits"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /gather"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /gather empty")
    echo -e "  ${RED}FAIL${NC} POST /gather empty"
fi
