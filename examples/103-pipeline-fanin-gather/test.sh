...
echo "=== 103 Pipeline Fan-In Gather ==="

# Credit pipeline (:3093)
if nc -z localhost 3093 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} Credit port 3093"
    RESP=$(curl -s --max-time 15 -X POST http://localhost:3093/gather -H 'Content-Type: application/json' \
      -d '{"request":"credits"}' 2>/dev/null)
    if [ -n "$RESP" ]; then
        assert_output_contains "$RESP" "_source" "gather has _source"
    else
        FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("Credit stream empty")
        echo -e "  ${RED}FAIL${NC} Credit stream empty"
    fi
else
    skip_test "Credit :3093 not open"
fi

# Inventory pipeline (:3094)
if nc -z localhost 3094 2>/dev/null; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} Inventory port 3094"
else
    skip_test "Inventory :3094 not open"
fi
