...
echo "=== 101 Pipeline 3-Node Transform Chain ==="

# Check credit-data-simulator
if ! curl -s http://localhost:18081/health 2>/dev/null | grep -q "healthy"; then
    echo -e "  ${YELLOW}SKIP${NC} credit-data-simulator not running on :18081"
    SKIP_COUNT=$((SKIP_COUNT + 1))
    return 0 2>/dev/null || exit 0
fi

# Transform — pipeline fetches NDJSON, applies 3-step transform chain
RESP=$(curl -s -X POST "$BASE/transform" -H 'Content-Type: application/json' \
  -d '{"request":"chain-transforms"}' --max-time 10)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "nama_lengkap" "transform has nama_lengkap"
    assert_output_contains "$RESP" "_risk_score" "transform has _risk_score (enrich step)"
    assert_output_contains "$RESP" "_risk_class" "transform has _risk_class (classify step)"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("101: POST /transform returned empty")
    echo -e "  ${RED}FAIL${NC} POST /transform empty response"
fi
