...
echo "=== 101-pipeline-3node-transform-chain (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /transform
RESP=$(curl -s --max-time 10 -X POST "$BASE/transform" -H 'Content-Type: application/json' -d '{"request":"chain-transforms"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /transform"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /transform empty")
    echo -e "  ${RED}FAIL${NC} POST /transform empty"
fi
