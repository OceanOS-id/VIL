...
# Usage: cargo run --release & sleep 2 && bash test_vwfd.sh
...
echo "=== 107-pipeline-process-traced (VWFD) ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /traced
RESP=$(curl -s --max-time 10 -X POST "$BASE/traced" -H 'Content-Type: application/json' -d '{"tracking_id":"PKG-001","origin":"JKT","destination":"SBY"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /traced"
else
    FAIL_COUNT=$((FAIL_COUNT+1)); FAILURES+=("POST /traced empty")
    echo -e "  ${RED}FAIL${NC} POST /traced empty"
fi
