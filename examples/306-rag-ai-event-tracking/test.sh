...
echo "=== 306 RAG AI Event Tracking ==="
...
# Quality stats (before query)
RESP=$(curl -s "$BASE/api/support/quality")
assert_output_contains "$RESP" "total_queries" "quality has total_queries"

# Support ask
RESP=$(curl -s -X POST "$BASE/api/support/ask" -H 'Content-Type: application/json' \
  -d '{"question":"How do I return a product?","top_k":3}' --max-time 15)
if [ -n "$RESP" ]; then
    assert_output_contains "$RESP" "answer" "support has answer"
    assert_output_contains "$RESP" "sources" "support has sources"
    assert_output_contains "$RESP" "quality" "support has quality metrics"
else
    FAIL_COUNT=$((FAIL_COUNT + 1))
    FAILURES+=("306: POST /api/support/ask empty")
    echo -e "  ${RED}FAIL${NC} POST /api/support/ask empty"
fi
