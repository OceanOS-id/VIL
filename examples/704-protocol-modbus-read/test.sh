...
echo "=== 704-protocol-modbus-read ==="

if [ -n "${VIL_CLI_LOG:-}" ] && [ -f "$VIL_CLI_LOG" ]; then
    EXIT=${VIL_CLI_EXIT:-1}
    if [ "$EXIT" -eq 0 ]; then
        PASS_COUNT=$((PASS_COUNT + 1))
        echo -e "  ${GREEN}PASS${NC} binary completed (exit=0)"
    else
        FAIL_COUNT=$((FAIL_COUNT + 1))
        FAILURES+=("704-protocol-modbus-read: exit=$EXIT")
        echo -e "  ${RED}FAIL${NC} binary exit=$EXIT"
    fi
