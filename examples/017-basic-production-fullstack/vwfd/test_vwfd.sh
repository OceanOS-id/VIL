...
echo "=== 017-basic-production-fullstack (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/platform/config
RESP=$(curl -s --max-time 10 "$BASE/api/platform/config")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/platform/config"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/platform/config empty")
    echo -e "  ${RED}FAIL${NC} GET /api/platform/config empty"
fi

# POST /api/sprints/create
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/sprints/create" -H 'Content-Type: application/json' -d '{"title":"Test Sprint","status":"planned","assignee":"dev","story_points":3}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/sprints/create"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/sprints/create empty")
    echo -e "  ${RED}FAIL${NC} POST /api/sprints/create empty"
fi

# GET /api/sprints/list
RESP=$(curl -s --max-time 10 "$BASE/api/sprints/list")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/sprints/list"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/sprints/list empty")
    echo -e "  ${RED}FAIL${NC} GET /api/sprints/list empty"
fi

# GET /api/sprints/stats
RESP=$(curl -s --max-time 10 "$BASE/api/sprints/stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/sprints/stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/sprints/stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/sprints/stats empty"
fi

# PUT /api/sprints/update
RESP=$(curl -s --max-time 10 -X PUT "$BASE/api/sprints/update" -H 'Content-Type: application/json' -d '{"title":"Updated","status":"in_progress"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} PUT /api/sprints/update"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("PUT /api/sprints/update empty")
    echo -e "  ${RED}FAIL${NC} PUT /api/sprints/update empty"
fi
