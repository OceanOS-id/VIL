...
echo "=== 901-faas-kyc-onboarding (VWFD) ==="
# Health
...
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/kyc/verify" -H 'Content-Type: application/json' -d '{"test":true}')
[ -n "$RESP" ] && { PASS_COUNT=$((PASS_COUNT+1)); echo -e "  ${GREEN}PASS${NC} POST /api/kyc/verify"; } \
    || { FAIL_COUNT=$((FAIL_COUNT+1)); echo -e "  ${RED}FAIL${NC} POST /api/kyc/verify empty"; }
