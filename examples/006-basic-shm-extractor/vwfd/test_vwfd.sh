...
echo "=== 006-basic-shm-extractor (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/shm-demo/benchmark
RESP=$(curl -s --max-time 10 "$BASE/api/shm-demo/benchmark")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/shm-demo/benchmark"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/shm-demo/benchmark empty")
    echo -e "  ${RED}FAIL${NC} GET /api/shm-demo/benchmark empty"
fi

# POST /api/shm-demo/compute
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/shm-demo/compute" -H 'Content-Type: application/json' -d '{"iterations":100000}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/shm-demo/compute"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/shm-demo/compute empty")
    echo -e "  ${RED}FAIL${NC} POST /api/shm-demo/compute empty"
fi

# POST /api/shm-demo/ingest
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/shm-demo/ingest" -H 'Content-Type: application/json' -d '{"prompt":"test","request":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/shm-demo/ingest"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/shm-demo/ingest empty")
    echo -e "  ${RED}FAIL${NC} POST /api/shm-demo/ingest empty"
fi

# GET /api/shm-demo/shm-stats
RESP=$(curl -s --max-time 10 "$BASE/api/shm-demo/shm-stats")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/shm-demo/shm-stats"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/shm-demo/shm-stats empty")
    echo -e "  ${RED}FAIL${NC} GET /api/shm-demo/shm-stats empty"
fi
