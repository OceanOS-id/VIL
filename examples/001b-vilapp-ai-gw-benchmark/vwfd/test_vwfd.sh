...
echo "=== 001b-vilapp-ai-gw-benchmark (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# POST /api/gw/stream
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/gw/stream" -H 'Content-Type: application/json' -d '{"test":true}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/gw/stream"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/gw/stream empty")
    echo -e "  ${RED}FAIL${NC} POST /api/gw/stream empty"
fi

# POST /trigger
RESP=$(curl -s --max-time 10 -X POST "$BASE/trigger" -H 'Content-Type: application/json' -d '{"prompt":"test","request":"test"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /trigger"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /trigger empty")
    echo -e "  ${RED}FAIL${NC} POST /trigger empty"
fi
