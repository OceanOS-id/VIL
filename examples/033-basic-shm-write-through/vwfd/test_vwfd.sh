...
echo "=== 033-basic-shm-write-through (VWFD) ==="

# Health (vil_vwfd::app always has /health)
assert_http_status "$BASE/health" 200 "health endpoint"

# GET /api/catalog/catalog/health
RESP=$(curl -s --max-time 10 "$BASE/api/catalog/catalog/health")
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} GET /api/catalog/catalog/health"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("GET /api/catalog/catalog/health empty")
    echo -e "  ${RED}FAIL${NC} GET /api/catalog/catalog/health empty"
fi

# POST /api/catalog/catalog/search
RESP=$(curl -s --max-time 10 -X POST "$BASE/api/catalog/catalog/search" -H 'Content-Type: application/json' -d '{"prompt":"What is API?"}')
if [ -n "$RESP" ]; then
    PASS_COUNT=$((PASS_COUNT + 1))
    echo -e "  ${GREEN}PASS${NC} POST /api/catalog/catalog/search"
else
    FAIL_COUNT=$((FAIL_COUNT + 1)); FAILURES+=("POST /api/catalog/catalog/search empty")
    echo -e "  ${RED}FAIL${NC} POST /api/catalog/catalog/search empty"
fi
