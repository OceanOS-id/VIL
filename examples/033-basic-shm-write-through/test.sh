...
echo "=== 033 SHM Write-Through ==="

# Health
assert_http_status "$BASE/health" 200 "health endpoint"

# Catalog health
assert_http_status "$BASE/api/catalog/catalog/health" 200 "catalog health"

# Search products (electronics under $500)
RESP=$(curl -s -X POST "$BASE/api/catalog/catalog/search" -H 'Content-Type: application/json' \
  -d '{"category":"electronics","max_price_cents":50000}')
assert_output_contains "$RESP" "products" "search has products"
assert_output_contains "$RESP" "products_returned" "search has products_returned"
assert_output_contains "$RESP" "shm_available" "search shows shm_available"
