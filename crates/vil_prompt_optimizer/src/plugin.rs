...
impl VilPlugin for PromptOptimizerPlugin {
    fn id(&self) -> &str { "vil-prompt-optimizer" }
    fn version(&self) -> &str { env!("CARGO_PKG_VERSION") }
    fn description(&self) -> &str { "Automated prompt engineering with candidate evaluation" }

    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![PluginCapability::Service {
            name: "prompt-optimizer".into(),
            endpoints: vec![
                EndpointSpec::get("/api/prompt-optimizer/stats"),
            ],
        }]
    }
    ...
    fn register(&self, ctx: &mut PluginContext) {
        ...
        let optimizer = Arc::new(RwLock::new(
            PromptOptimizer::new(evaluator, OptimizeStrategy::GridSearch),
        ));
