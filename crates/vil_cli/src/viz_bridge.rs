...
pub fn run_viz(args: VizArgs) -> Result<(), String> {
    ...
    if args.expand_calls {
        ...
        if let Ok(resolved) = crate::call_resolver::resolve_all_calls(&manifest, base_dir) {
    ...
    for (name, node) in &manifest.nodes {
        let node_type = match node.node_type.as_str() {
            ...
            other => {
                ...
                if crate::node_types::find_node_type(other).is_some() {
...
fn run_call_graph_viz(dir: &str, args: &VizArgs) -> Result<(), String> {
    let call_graph = crate::call_resolver::scan_call_graph(std::path::Path::new(dir))?;
...
fn call_to_subgraph(call_path: &str, resolved: &crate::call_resolver::ResolvedCall) -> VizSubgraph {
    let mut nodes = Vec::new();
    let mut edges = Vec::new();
