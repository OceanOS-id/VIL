...
pub fn install_sdk(version: &str) -> Result<(), String> {
    ...
    if local_sdk.exists() && local_sdk.join("libs").exists() {
        println!(
            "  {} Found local sdk-dist/, installing from local source",
            "OK".green()
        );
    ...
    println!(
        ...
        version_dir.join("internal").display()
    );
    println!("  {} current -> {}", "LINK".dimmed(), version_dir.display());

    if libs_ok && internal_ok {
        println!(
            "\n{} SDK ready. `vil compile` will use pre-compiled engine.",
            "DONE".green().bold()
        );
    } else {
        println!(
            "\n{} SDK partially installed — some directories missing.",
            "WARN".yellow().bold()
        );
    }
...
pub fn show_info() -> Result<(), String> {
    ...
    if libs.exists() {
        ...
        println!(
            "  libvil_engine.a:  {} ({:.1} MB)",
            if engine_a.exists() {
                "OK".green()
            } else {
                "MISSING".red()
            },
            engine_size as f64 / 1_048_576.0
        );
        println!(
            "  libvil_runtime.a: {} ({:.1} MB)",
            if runtime_a.exists() {
                "OK".green()
            } else {
                "MISSING".red()
            },
            runtime_size as f64 / 1_048_576.0
        );
    } else {
...
pub fn list_versions() -> Result<(), String> {
    ...
    let entries = std::fs::read_dir(&base).map_err(|e| format!("Failed to read SDK dir: {}", e))?;
