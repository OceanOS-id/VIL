...
use crate::{JsonOutputParser, MarkdownOutputParser, RegexOutputParser, OutputParser, ParsedOutput};
...
pub async fn parse_handler(
    ...
    let parsed: ParsedOutput = match req.format.as_str() {
        ...
        "regex" => {
            let pattern = req.pattern.as_deref()
        ...
        other => {
            return Err(VilError::bad_request(format!("unsupported format: {other}")));
        }
    };
    ...
    let value = match &parsed {
        ...
        ParsedOutput::Structured(map) => serde_json::to_value(map)
