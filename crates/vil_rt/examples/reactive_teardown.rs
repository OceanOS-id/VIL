...
use vil_types::{
    BoundaryKind, CleanupPolicy, ControlSignal, ExecClass, HostId, LayoutProfile, MessageContract,
    MessageMeta, ObservabilitySpec, PortDirection, PortSpec, ProcessSpec, QueueKind, TransferMode,
};
...
fn main() -> std::io::Result<()> {
    ...
    static PORTS_A: &[PortSpec] = &[
        PortSpec {
            ...
            boundary: BoundaryKind::IntraProcess,
            timeout_ms: None,
        ...
        PortSpec {
            ...
            boundary: BoundaryKind::IntraProcess,
            timeout_ms: None,
            priority: vil_types::Priority::High,
            delivery: vil_types::DeliveryGuarantee::AtLeastOnce,
            observability: ObservabilitySpec {
                tracing: true,
                metrics: true,
                lineage: true,
    ...
    // --- Wiring ---
    world.connect(
        proc_a.port_id("data_out").unwrap(),
        proc_b.port_id("data_in").unwrap(),
    );
    world.connect(
        proc_a.port_id("ctrl_out").unwrap(),
        proc_b.port_id("ctrl_in").unwrap(),
    );
    world.connect(
        proc_b.port_id("data_out").unwrap(),
        proc_c.port_id("data_in").unwrap(),
    );
    world.connect(
        proc_b.port_id("ctrl_out").unwrap(),
        proc_c.port_id("ctrl_in").unwrap(),
    );
    ...
    loop {
        if let Ok(data) = world.recv::<DataPoint>(proc_b.port_id("data_in").unwrap()) {
            ...
            if let ControlSignal::Done { session_id } = ctrl {
                println!(
                    "✅ Node B received DONE for session {}, finished {} samples. Propagating...",
                    session_id, count_b
                );
                world
                    .publish_control(
    ...
    loop {
        if let Ok(_data) = world.recv::<DataPoint>(proc_c.port_id("data_in").unwrap()) {
            ...
            if let ControlSignal::Done { session_id } = ctrl {
                println!(
                    "🏆 Node C received DONE for session {}, total samples processed: {}",
                    session_id, count_c
                );
                break;
            }
