...
impl VilPlugin for PrivateRagPlugin {
    fn id(&self) -> &str { "vil-private-rag" }
    fn version(&self) -> &str { env!("CARGO_PKG_VERSION") }
    fn description(&self) -> &str { "Privacy-preserving RAG with PII redaction and audit logging" }

    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![PluginCapability::Service {
            name: "private-rag".into(),
            endpoints: vec![
                EndpointSpec::get("/api/private-rag/stats"),
            ],
        }]
    }
