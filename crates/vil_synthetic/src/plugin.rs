...
impl VilPlugin for SyntheticPlugin {
    fn id(&self) -> &str { "vil-synthetic" }
    fn version(&self) -> &str { env!("CARGO_PKG_VERSION") }
    fn description(&self) -> &str { "Template-based synthetic data generation with quality checking" }

    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![PluginCapability::Service {
            name: "synthetic".into(),
            endpoints: vec![
                EndpointSpec::get("/api/synthetic/stats"),
            ],
        }]
    }
    ...
    fn register(&self, ctx: &mut PluginContext) {
        let gen = Arc::new(SyntheticGenerator::new(
            vec![qa_template(), instruction_template(), conversation_template()],
