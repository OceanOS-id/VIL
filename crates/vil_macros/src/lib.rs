...
pub fn message(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    let memory_class_tokens = if let Some(eq_pos) = attr_str.find("memory_class") {
        ...
        let variant = after.split('=').nth(1).unwrap_or("").trim().trim_matches(',').trim().to_string();
        let valid = ["PagedExchange", "PinnedRemote", "ControlHeap", "LocalScratch"];
...
fn generate_semantic_type(
    ...
    let mut builder = vil_ir::builder::MessageBuilder::new(&name_str)
        .semantic_kind(semantic_kind_ir);
    ...
    workflow_ir.messages.insert(name_str.clone(), msg_ir.clone());

    let validator = vil_validate::LayoutLegalityPass;
    let report = vil_validate::ValidationPass::run(&validator, &workflow_ir);

    if report.has_errors() {
        let error_msg = report.diagnostics.iter()
            .map(|d| format!("[VIL {}] {}", d.code, d.message))
            .collect::<Vec<_>>()
            .join("\n");
        return syn::Error::new_spanned(name, error_msg).to_compile_error().into();
    }
...
fn syn_type_to_ir(ty: &syn::Type) -> vil_ir::core::TypeRefIR {
    ...
    } else if ["u8", "u16", "u32", "u64", "i8", "i16", "i32", "i64", "f32", "f64", "bool"].contains(&type_str.as_str()) {
...
pub fn process(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if let Some(eq_pos) = attr_string.find("zone") {
        ...
        let zone_str = after.split('=').nth(1).unwrap_or("").trim().trim_matches(',').trim().to_string();
        if ["NativeCore", "NativeTrusted", "WasmCapsule", "ExternalBoundary"].contains(&zone_str.as_str()) {
    ...
    let has_trace_hop = input.attrs.iter().any(|a| {
        a.path().get_ident().map(|id| id == "vil_obs_trace_hop").unwrap_or(false)
    });
    if has_trace_hop {
        trace_hop_tokens = quote! { .obs_trace_hop() };
    }
    ...
    for a in &input.attrs {
        if a.path().get_ident().map(|id| id == "vil_obs_latency_label").unwrap_or(false) {
    ...
    clean.attrs.retain(|a| {
        ...
        !matches!(id.as_deref(), Some("vil_obs_trace_hop") | Some("vil_obs_latency_label"))
    });
...
pub fn vil_workflow(input: TokenStream) -> TokenStream {
    ...
    let failovers_calls = def.failovers.iter().map(|f| {
        ...
        let target = if f.target == "retry" {
            let attempts = f.retry_attempts.as_ref().map(|a| a.base10_parse::<u32>().unwrap_or(3)).unwrap_or(3);
            let backoff = f.retry_backoff.as_ref().map(|b| b.base10_parse::<u64>().unwrap_or(100)).unwrap_or(100);
            format!("retry({}, {}ms)", attempts, backoff)
        } else {
            f.target.to_string()
        };

        let strategy = f.strategy.as_ref().map(|s| s.to_string()).unwrap_or_else(|| "ImmediateBackup".to_string());
    ...
    for r in &def.routes {
        ...
        if is_src_instance && is_dst_instance {
            let src_handle = syn::Ident::new(&format!("{}_handle", r.src_process), r.src_process.span());
            let dst_handle = syn::Ident::new(&format!("{}_handle", r.dst_process), r.dst_process.span());
            let src_port = r.src_port.to_string();
            let dst_port = r.dst_port.to_string();
