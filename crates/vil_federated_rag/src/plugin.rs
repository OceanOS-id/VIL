...
impl VilPlugin for FederatedRagPlugin {
    fn id(&self) -> &str { "vil-federated-rag" }
    fn version(&self) -> &str { env!("CARGO_PKG_VERSION") }
    fn description(&self) -> &str { "Cross-silo federated retrieval with score-based merging" }

    fn capabilities(&self) -> Vec<PluginCapability> {
        vec![PluginCapability::Service {
            name: "federated-rag".into(),
            endpoints: vec![
                EndpointSpec::get("/api/federated-rag/stats"),
            ],
        }]
    }
