...
pub async fn stats_handler(
    ctx: ServiceCtx,
) -> HandlerResult<VilResponse<FederatedStatsBody>> {
    let retriever = ctx.state::<Arc<FederatedRetriever>>().expect("FederatedRetriever");
    Ok(VilResponse::ok(FederatedStatsBody {
        source_count: retriever.sources.len(),
        merge_strategies: vec!["score_interleave".into(), "round_robin".into(), "dedup".into()],
