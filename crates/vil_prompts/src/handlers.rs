...
pub async fn render_handler(
    ...
    let registry = ctx
        .state::<Arc<RwLock<PromptRegistry>>>()
    ...
    let rendered = reg
        .render(&req.template_name, &req.variables)
...
pub async fn list_handler(ctx: ServiceCtx) -> HandlerResult<VilResponse<PromptListResponseBody>> {
    let registry = ctx
        .state::<Arc<RwLock<PromptRegistry>>>()
        .expect("PromptRegistry");
    let reg = registry
        .read()
        .map_err(|_| VilError::internal("lock poisoned"))?;
    let templates: Vec<PromptListEntry> = reg
        .names()
        .map(|name| {
