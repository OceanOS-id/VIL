use std::fmt::Write;
use vil_ir::{MessageIR, TypeRefIR, WorkflowIR};
...
pub fn generate_header(ir: &WorkflowIR) -> String {
    ...
    if !states.is_empty() {
        ...
        for msg in &states {
            writeln!(
                header,
                "/* semantic_kind: state — mutable per-session, Data Lane only */"
            )
            .unwrap();
            writeln!(header, "{}", generate_struct(msg)).unwrap();
        }
    ...
    if !faults.is_empty() {
        ...
        writeln!(
            header,
            "/* Note: Faults in VIL are enums — represented as tagged union in C */"
        )
        .unwrap();
        for msg in &faults {
...
mod tests {
    use super::*;
    use vil_ir::builder::{InterfaceBuilder, MessageBuilder, WorkflowBuilder};
    ...
    fn test_semantic_kind_grouping() {
        let ir = WorkflowBuilder::new("SemanticTest")
            .add_message(
                MessageBuilder::new("SessionProgress")
            ...
            .add_message(
                MessageBuilder::new("ProcessingError")
